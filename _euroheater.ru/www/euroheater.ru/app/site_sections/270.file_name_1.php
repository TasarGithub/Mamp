<img class="brand_logo" src="/public/images/wilson.jpg" />
          <p>Предлагаем в аренду электростанции Wilson P500 по разумной цене – для крупного строительства или производства. В случае длительной аренды составляем индивидуальное коммерческое предложение.</p>
<p>В аренду генератора Wilson P500 входит доставка на объекты в Москве и области, НДС, работа оператора, заправка топливом и оформление всех необходимых документов.</p>

<br />

<div class="item_col_1">
    <img class="img-responsive" src="/public/images/catalog/p500.jpg" alt="Wilson P500 " title="Wilson P500" /><br />

    <div class="cat_price">от 9500<span>руб./сутки</span></div>
    посуточная оплата без топлива

    <a class="order_btn" href="#rent_modal" data-toggle="modal">Отправить заявку</a>
    <br /><br />
</div>

<div class="item_col_2">
    <div class="h4">Технические характеристики</div>
    <table class="char_table">
                <tr>
            <td>Производитель</td>
            <td>Wilson (Китай)</td>
        </tr>
        
                <tr>
            <td>Основная мощность</td>
            <td>455 кВА / 364 кВт</td>
        </tr>
        
                <tr>
            <td>Резервная мощность</td>
            <td>500 кВА / 400 кВт</td>
        </tr>
        
                <tr>
            <td>Расход топлива при нагрузке 75%</td>
            <td>71,8 л/ч</td>
        </tr>
        
                <tr>
            <td>Обороты двигателя</td>
            <td>1500 об/мин, пониженные</td>
        </tr>
        
                <tr>
            <td>Выходное напряжение</td>
            <td>380/220В (3-фазн)</td>
        </tr>
        
                <tr>
            <td>Генератор</td>
            <td>Leroy Somer (Франция)</td>
        </tr>
        
                <tr>
            <td>Двигатель</td>
            <td>Perkins (Великобритания)</td>
        </tr>
        
                <tr>
            <td>Тип топлива</td>
            <td>дизель</td>
        </tr>
        
                <tr>
            <td>Объем топливного бака</td>
            <td>915 л</td>
        </tr>
        
                <tr>
            <td>Тип запуска</td>
            <td>электро / автоматический</td>
        </tr>
        
                <tr>
            <td>Масса</td>
            <td>5106 кг</td>
        </tr>
        
                <tr>
            <td>Габариты</td>
            <td>493 x 165 x 214 см</td>
        </tr>
        
                <tr>
            <td>Исполнение</td>
            <td>в кожухе</td>
        </tr>
        
                <tr>
            <td>Класс электростанции</td>
            <td>промышленная стационарная</td>
        </tr>
        
                <tr>
            <td>Тип охлаждения</td>
            <td>жидкостное</td>
        </tr>
            </table>
</div>

<div class="clear"></div><br />

<div class="h4 collapse_sm">Тарифы на аренду</div>

<table class="price_table_1 collapse_sm">
    <tr>
<td colspan="2" class="td_1" width="280">
    <table>
    <tr>
    <td colspan="2">Почасовая оплата<br />«все включено»</td>
</tr>
    <tr>
    <td>в режиме работы</td>
<td>в режиме простоя</td>
</tr>
</table>
</td>
<td width="140">Посуточная оплата электростанции без топлива</td>
<td width="140">Посуточная оплата услуг нашего оператора</td>
<td width="105">Режим простоя с оператором в сутки</td>
<td width="105">Режим простоя без оператора в сутки</td>
<td></td>
</tr>
    <tr>
<td>от 2840 руб./час</td>
<td>от 500 руб./час</td>
<td>от 9500 руб./сутки</td>
<td>от 2500 руб./сутки</td>
<td>от 12000 руб.</td>
<td>от 5960 руб.</td>
<td><a href="#calc_modal" class="" data-toggle="modal">Рассчитать</a></td>
</tr>
</table>

<p>Чтобы рассчитать прокат электростанции, а также задать вопросы по характеристикам генераторов, свяжитесь с нашими специалистами: <?php echo $GLOBALS['phone']; ?>.</p>
<br />

<a class="order_btn_1 icon_2" href="/public/docs/2_euroheater.ru_dogovor.docx">Скачать договор на аренду</a>

<a class="page_back" href="/generatora/">&laquo; на страницу "Оборудование"</a><br />

<div class="h4">Другие модели электростанций</div>

<!-- Электростанция -->
<div class="cat_item">
    <div class="cat_col_1">
        <a href="/generatora-wilson/p135/"><img src="/public/images/catalog/p135-small.jpg" /></a>
    </div>
    <div class="cat_col_2">
        <div class="cat_t"><a href="/generatora-wilson/p135/">Wilson P135</a></div>
        <div>Ном. мощность: 136 кВА / 109 кВт</div>
        <div>Резервная мощность: 149 кВА / 119 кВт</div>
        <div>Модель двигателя: Perkins (Великобритания)</div>
        <div>Расход топлива при 75% загрузки: 23,4 л/ч</div>
    </div> 
    <div class="cat_col_3">
        <div class="cat_price">от 4000 <span>руб./сутки</span></div>
        <div class="txt_1">посуточная оплата без топлива</div>
        <a class="order_btn_1 btn_md" href="#calc_modal" data-toggle="modal">Рассчитать стоимость</a>
    </div>
    <div class="clear"></div>
</div>
<!-- Электростанция -->

<!-- Электростанция -->
<div class="cat_item">
    <div class="cat_col_1">
        <a href="/generatora-wilson/p400/"><img src="/public/images/catalog/p400-small.jpg" /></a>
    </div>
    <div class="cat_col_2">
        <div class="cat_t"><a href="/generatora-wilson/p400/">Wilson P400</a></div>
        <div>Ном. мощность: 363 кВА / 291 кВт</div>
        <div>Резервная мощность: 399 кВА / 320 кВт</div>
        <div>Модель двигателя: Perkins (Великобритания)</div>
        <div>Расход топлива при 75% загрузки: 61 л/ч</div>
    </div> 
    <div class="cat_col_3">
        <div class="cat_price">от 8500 <span>руб./сутки</span></div>
        <div class="txt_1">посуточная оплата без топлива</div>
        <a class="order_btn_1 btn_md" href="#calc_modal" data-toggle="modal">Рассчитать стоимость</a>
    </div>
    <div class="clear"></div>
</div>
<!-- Электростанция -->

<br><br>
<a class="order_btn_2 icon_1" href="/public/docs/1_euroheater.ru_price.doc">Скачать прайс-лист</a>
<a class="order_btn_1 icon_2" href="/public/docs/2_euroheater.ru_dogovor.docx">Скачать договор на аренду</a>
<br>