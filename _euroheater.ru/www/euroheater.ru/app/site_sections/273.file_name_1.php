<img class="brand_logo" src="/public/images/denyo.jpg" />
          <p>Предоставляем в аренду генераторы Denyo DCA-150 ESK и аналоги на срок от 3 дней с доставкой по Москве и Подмосковью. Электростанции привозим круглосуточно. Обеспечиваем топливом и кабелем для подключения.</p>

<br />

<div class="item_col_1">
    <img class="img-responsive" src="/public/images/catalog/dca150.png" alt="Denyo DCA-150ESK" title="Denyo DCA-150ESKI" /><br />

    <div class="cat_price">от 4000 <span>руб./сутки</span></div>
    посуточная оплата без топлива

    <a class="order_btn" href="#rent_modal" data-toggle="modal">Отправить заявку</a>
    <br /><br />
</div>

<div class="item_col_2">
    <div class="h4">Технические характеристики</div>
    <table class="char_table">
                <tr>
            <td>Производитель</td>
            <td>Denyo  (Япония)</td>
        </tr>
        
                <tr>
            <td>Основная мощность</td>
            <td>125 кВА / 100 кВт</td>
        </tr>
        
                <tr>
            <td>Резервная мощность</td>
            <td>137 кВА / 110 кВт</td>
        </tr>
        
                <tr>
            <td>Расход топлива при нагрузке 75%</td>
            <td>20,6 л/ч</td>
        </tr>
        
                <tr>
            <td>Обороты двигателя</td>
            <td>1500 об/мин, пониженные</td>
        </tr>
        
                <tr>
            <td>Выходное напряжение</td>
            <td>400/230В (3-фазн)</td>
        </tr>
        
                <tr>
            <td>Генератор</td>
            <td>Komatsu (Япония)</td>
        </tr>
        
                <tr>
            <td>Двигатель</td>
            <td>Isuzu (Япония)</td>
        </tr>
        
                <tr>
            <td>Тип топлива</td>
            <td>дизель</td>
        </tr>
        
                <tr>
            <td>Объем топливного бака</td>
            <td>250 л</td>
        </tr>
        
                <tr>
            <td>Тип запуска</td>
            <td>электро / автоматический</td>
        </tr>
        
                <tr>
            <td>Масса</td>
            <td>2390 кг</td>
        </tr>
        
                <tr>
            <td>Габариты</td>
            <td>325 x 108 x 150 см</td>
        </tr>
        
                <tr>
            <td>Исполнение</td>
            <td>в кожухе</td>
        </tr>
        
                <tr>
            <td>Класс электростанции</td>
            <td>промышленная стационарная</td>
        </tr>
        
                <tr>
            <td>Тип охлаждения</td>
            <td>жидкостное</td>
        </tr>
            </table>
</div>

<div class="clear"></div><br />

<div class="h4 collapse_sm">Тарифы на аренду</div>

<table class="price_table_1 collapse_sm">
    <tr>
<td colspan="2" class="td_1" width="280">
    <table>
    <tr>
    <td colspan="2">Почасовая оплата<br />«все включено»</td>
</tr>
    <tr>
    <td>в режиме работы</td>
<td>в режиме простоя</td>
</tr>
</table>
</td>
<td width="140">Посуточная оплата электростанции без топлива</td>
<td width="140">Посуточная оплата услуг нашего оператора</td>
<td width="105">Режим простоя с оператором в сутки</td>
<td width="105">Режим простоя без оператора в сутки</td>
<td></td>
</tr>
    <tr>
<td>от 805 руб./час</td>
<td>от 215 руб./час</td>
<td>от 4000 руб./сутки</td>
<td>от 2500 руб./сутки</td>
<td>от 5160 руб.</td>
<td>от 2760 руб.</td>
<td><a href="#calc_modal" class="" data-toggle="modal">Рассчитать</a></td>
</tr>
</table>

<p>Бесплатно консультируем клиентов по подбору оборудования. Предоставляем опытного оператора для постоянного нахождения на объекте. Если техника выйдет из строя, бесплатно меняем ее на аналогичную в течение 24 часов.</p>
<p>Чтобы рассчитать цену аренды с учетом дополнительных услуг и доставки, свяжитесь с нами по телефону <?php echo $GLOBALS['phone']; ?>.</p>
<br />

<a class="order_btn_1 icon_2" href="/public/docs/2_euroheater.ru_dogovor.docx">Скачать договор на аренду</a>

<a class="page_back" href="/generatora/">&laquo; на страницу "Оборудование"</a><br />

<div class="h4">Другие модели электростанций</div>

<!-- Электростанция -->
<div class="cat_item">
    <div class="cat_col_1">
        <a href="/generatora-denyo/dca45/"><img src="/public/images/catalog/dca45-small.png" /></a>
    </div>
    <div class="cat_col_2">
        <div class="cat_t"><a href="/generatora-denyo/dca45/">Denyo 45 ESI</a></div>
        <div>Ном. мощность: 37 кВА / 30 кВт</div>
        <div>Резервная мощность: 40 кВА / 33 кВт</div>
        <div>Модель двигателя: Isuzu  (Япония)</div>
        <div>Расход топлива при 75% загрузки: 6,7 л/ч</div>
    </div> 
    <div class="cat_col_3">
        <div class="cat_price">от 1800 <span>руб./сутки</span></div>
        <div class="txt_1">посуточная оплата без топлива</div>
        <a class="order_btn_1 btn_md" href="#calc_modal" data-toggle="modal">Рассчитать стоимость</a>
    </div>
    <div class="clear"></div>
</div>
<!-- Электростанция -->

<!-- Электростанция -->
<div class="cat_item">
    <div class="cat_col_1">
        <a href="/generatora-denyo/dca300/"><img src="/public/images/catalog/dca150-small.png" /></a>
    </div>
    <div class="cat_col_2">
        <div class="cat_t"><a href="/generatora-denyo/dca300/">Denyo DCA 300 SPK3 </a></div>
        <div>Ном. мощность: 270 кВА / 216 кВт</div>
        <div>Резервная мощность: 297 кВА / 237 кВт</div>
        <div>Модель двигателя: Komatsu (Япония)</div>
        <div>Расход топлива при 75% загрузки: 43,6 л/ч</div>
    </div> 
    <div class="cat_col_3">
        <div class="cat_price">от 6700 <span>руб./сутки</span></div>
        <div class="txt_1">посуточная оплата без топлива</div>
        <a class="order_btn_1 btn_md" href="#calc_modal" data-toggle="modal">Рассчитать стоимость</a>
    </div>
    <div class="clear"></div>
</div>
<!-- Электростанция -->

<br><br>
<a class="order_btn_2 icon_1" href="/public/docs/1_euroheater.ru_price.doc">Скачать прайс-лист</a>
<a class="order_btn_1 icon_2" href="/public/docs/2_euroheater.ru_dogovor.docx">Скачать договор на аренду</a>
<br>